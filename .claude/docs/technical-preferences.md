...
- **Ad-hoc RNG sources in simulation code** — `System.Random`, Godot's `RandomNumberGenerator`, Guid-derived entropy, or any RNG other than a named `SeededRngRegistry` stream are banned in the sim path; every draw asserts both MutationWindow clauses (window open AND `Publish` not on the stack) (ADR-0005)
- **Draws from the bare `RngOwner.Combat` root stream** — combat draws use named `SubKey` streams from day one; moving draws between streams is a stream-layout change guarded by the in-snapshot layout version, never a silent refactor (ADR-0005)
...
- **ADR-0005** (**Proposed** 2026-08-07; authored via `/architecture-decision`, review mode full): Seeded RNG / Determinism — hand-rolled PCG32 (16-byte explicit state per stream) in `Hollowdeep.Core`; single `WorldSeed` + stable `(RngOwner, SubKey)` key derivation (FNV-1a → SplitMix64; never registration order); fixed owner registry (`ColonistIdentity`, `RaidTrigger`, `Combat`, `MapGeneration`) with namespaced Combat `SubKey` extensibility and the bare-root-draw ban; draw-site assert reuses BOTH MutationWindow clauses (window open + `Publish` not on stack), with the load-window draws (`ColonistIdentity` embark spawn, `MapGeneration`) named as sanctioned; `Snapshot()` (colony saves) / `SnapshotInto(IBufferWriter<byte>)` (checkpoint path, discharging ADR-0004 item 7) / `Restore()`, with a stream-layout version that loud-fails battle-checkpoint loads across layout-changing patches; **the checkpoint captures the ENTIRE registry, never just Combat streams**; `NextRange` (Lemire debiased multiply) and `NextFloat01` (24-bit mantissa) pinned as format contract; streams are sealed reference types behind a composition-root-owned, per-world registry (never static). Gates: godot-specialist CONCERNS (3, folded); TD-ADR CONCERNS (C1–C6, all applied). Promotion NOT gated on the target-hardware run (RNG is not on the frame-time profile). Together with ADR-0004, unblocks Save/Load #6. See `docs/architecture/adr-0005-seeded-rng-determinism.md`.
- **Next**: `/propagate-design-change time-authority-mode-switch` (land CD-19/CD-20: the GDD honest-restatement paragraph, Rule 8/AC-10 catch-up ordering revision, AC-44 warning-window extension); then Save/Load quick-spec #6 (fully unblocked — ADR-0004 + ADR-0005 + CD-20 all landed) and Needs & Simulation GDD #13 (unblocked by CD-19: turn-derived-with-cap post-battle time, user ruling 2026-08-07); Colonist Entity quick-spec; target-hardware frame-rate run to promote ADR-0002 and ADR-0004.
