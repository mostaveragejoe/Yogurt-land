...
- **ADR-0005** (**Proposed** 2026-08-07; authored via `/architecture-decision`, review mode full): Seeded RNG / Determinism — a project-owned **PCG** generator in `Hollowdeep.Core` (pinned; explicitly not `System.Random` or Godot `RandomNumberGenerator`); **named per-system streams** constant-derived from one `masterSeed` (independent — a new draw site never shifts another stream); **full generator state serialized** (never seed+count → resumable at arbitrary draw counts, satisfying ADR-0004 item 3); **unidirectional Colony/Encounter scope firewall** — Encounter streams checkpoint-only, Colony streams in both saves (checkpoint is a full self-contained save); Lemire-unbiased bounded draws; draws only in `Tick()`/authority-driven resolution + the world-creation/load bootstrap window; a scope/authority draw-side invariant; CI grep gate against ad-hoc RNG in core. Gates: godot-specialist PASS WITH NOTES (folded); TD-ADR CONCERNS → both blocking (checkpoint carries colony streams; bootstrap-window draw context) + advisories applied. Blocking for Save/Load #6; completes ADR-0004's AC-67 (RNG half). See `docs/architecture/adr-0005-seeded-rng-determinism.md`.
- **Next**: cross-cutting contracts annex is done; remaining ADR/spec work — Save/Load #6 quick-spec (now unblocked by ADR-0004 + ADR-0005); the target-hardware frame-rate run gating ADR-0002/0004 promotion. Design-side: independent `/design-review` of the Time Authority GDD; the #8 Colonist Entity quick-spec.
