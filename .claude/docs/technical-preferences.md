...
- **ADR-0004** (**pending** — to be authored via `/architecture-decision`): Battle Checkpoint Architecture — owns checkpoint content scope, per-activation cadence, the Option A write mechanism (full self-contained checkpoint, double-buffered snapshot, async gzip+write — user decision 2026-08-03), the `RestoredFromCheckpoint` resume path, and the ordering/presentation invariants. See `docs/architecture/change-impact-2026-08-03-time-authority-mode-switch.md`.
- **Next**: ADR-0004 Battle Checkpoint Architecture; Seeded RNG ADR (constrained by ADR-0001's draws-only-inside-Tick rule; **now blocking for Save/Load #6** — streams must resume at arbitrary draw counts); cross-cutting contracts annex.
