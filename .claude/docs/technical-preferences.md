...
- **Holding the checkpoint buffer-handoff lock across I/O** — the lock covers the three-state buffer swap (*free*/*pending*/*in-flight*) ONLY; gzip, write, flush and the atomic replace all run outside it. Holding it across I/O makes the sim wait on disk, silently breaking the one requirement the whole async design exists to satisfy. Debug-asserted via a `[ThreadStatic]` lock-depth counter (ADR-0004 Amendment 2026-08-26, QQ-26)
- **An unbounded `Thread.Join()` on the checkpoint writer** — both the battle-end quiesce and the quit-path flush-and-join are bounded by `CheckpointJoinTimeout` (default 5 s); on timeout the temp file is abandoned, which is safe because atomic replace leaves the previous valid slot live. A stalled disk must never hang the process on quit (ADR-0004 Amendment 2026-08-26)
- **Handing a checkpoint content owner the file-level buffer** — each of the 7 owners receives a *section-scoped* writer and never sees the shared buffer; a raw `IBufferWriter<byte>` means one owner's overrun silently corrupts the next owner's section (ADR-0004 Amendment 2026-08-26, QQ-25)
...
- **Next** *(refreshed 2026-08-26 at `/architecture-review`)*: **amend ADR-0001** for the Pathfinding registration conflict (its tickable table contradicts the Pathfinding quick-spec, which registers no `ITickable`); **define the composition root** (QQ-24 — 30 references, 0 definitions, blocks the first store implementation and ADR-0006); **build ADR-0004's async checkpoint path so it can be measured (QQ-02)** — now unblocked, since QQ-25 (container framing) and QQ-26 (lock boundary + join timeout) closed 2026-08-26; it jointly gates ADR-0004 + ADR-0005 promotion and needs a run on target hardware, not a headless one; Save/Load quick-spec #6.
