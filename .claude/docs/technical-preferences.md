...
- **Next**: Seeded RNG ADR (constrained by ADR-0001's draws-only-inside-Tick rule; **now blocking for Save/Load #6** — streams must resume at arbitrary draw counts). All four foundation ADRs (0001–0004) are now Accepted. The only open ADR obligation is the shared criterion-5 target-hardware run, tracked as a pre-vertical-slice verification item.
