...
- **`System.Random` or Godot `RandomNumberGenerator` in the simulation path** — all gameplay draws go through `RngService`/`IRngStream`; `System.Random` is version-unstable and non-serializable, Godot RNG breaks the zero-Godot core rule (ADR-0005)
- **Floating-point RNG draws in `Hollowdeep.Core` or any `Tick()`-reachable path** — sim draws are integer-only (Lemire bounded); floats are not bit-identical across x64/ARM. A float helper, if ever needed, lives in the presentation assembly only (ADR-0005)
- **A single global RNG stream** — each system draws an independent stream keyed by `RngStreamId`; a shared stream couples draw order and makes determinism fragile (ADR-0005)
- **Drawing an RNG stream a system was not granted** — `RngService` hands out narrow per-stream `IRngStream` handles at the composition root; no draw-any-stream method exists; `Combat` draws assert `Mode == TurnBased` (ADR-0005)
- **RNG seed derivation without a domain tag** — stream seeds (`DOMAIN_STREAM`) and per-entity seeds (`DOMAIN_ENTITY`) must never share an un-tagged mixer, or small-integer ids collide (ADR-0005)
...
- **ADR-0002** (**Accepted** 2026-08-07, user decision; proposed 2026-07-24; spike-validated 2026-07-25; amended 2026-08-03 — Battle Persistence): Terrain Data Model — chunked dense grid of packed 8-byte `TerrainCell` structs (AoS, per-layer 32×32 chunks) behind a single `TerrainWorld` write facade; batched change events with previous-state capture (CD-1); `ApplyWallRepair` (CD-7); material manifest + schema version for stable-ID saves; writer set per time authority; mutation-window assertion; God-object firewall table. Plain C#, zero Godot dependency; GridMap is a candidate render backend only. Chunk size **32 confirmed**, AoS concession retired (AoS measured faster than SoA), snapshot = one-shot allocation, render backend = **two stacked GridMaps at octant 32**. **Promoted to Accepted 2026-08-07**: every measurable criterion passed (32 draw calls vs 150 budget, 16 MB video memory, ~2 µs/dig); the frame-rate clause of criterion 5 needs a real GPU that CI lacks, so it is now a pre-vertical-slice verification item, not a promotion gate. See `docs/architecture/adr-0002-terrain-data-model.md`.
...
- **ADR-0004** (**Accepted** 2026-08-07, user decision; authored 2026-08-03 via `/architecture-decision`, review mode full): Battle Checkpoint Architecture — checkpoint content scope (8 items), snapshot beat pinned at `AwaitingPresentation → NextActor` plus an "activation 0" checkpoint post-swap, the Option A write mechanism (full self-contained checkpoint, double-buffered pooled snapshot via a `SnapshotInto` caller-buffer obligation on ADR-0002/0003, coalesce-newest backpressure, async gzip + atomic same-volume replace), writer quiesce at slot retirement + in-file monotonic save ordering (never mtime), the `RestoredFromCheckpoint` resume path (no `RequestSwitch`; restore = distinct sanctioned writer in the load window; occupancy rebuild filters dead), write-side AC-68 provenance enforcement (writer-id header), quit-path flush-and-join requirement, and loud-fallback corrupt-checkpoint recovery. Gates: godot-specialist PASS WITH NOTES (folded); TD-ADR CONCERNS → all B1–B7 + A1–A10 applied. Promoted to Accepted 2026-08-07 with ADR-0002; the shared criterion-5 target-hardware run is now a pre-vertical-slice verification item, not a promotion gate. See `docs/architecture/adr-0004-battle-checkpoint-architecture.md`.
- **ADR-0005** (**Accepted** 2026-08-07, user decision; authored 2026-08-07 via `/architecture-decision`, review mode full): Seeded RNG — **xoshiro256\*\*** for every long-lived stream (32-byte state = position, so arbitrary-draw-count resume needs no counter), **SplitMix64** as seed mixer only. One plain-C# `RngService` owns and serializes all streams, keyed by a fixed `RngStreamId` enum (`Spawn`/`Combat`/`RaiderAI`/`MapGen`), seeded from a per-colony 64-bit master seed with **domain separation** (`DOMAIN_STREAM` vs `DOMAIN_ENTITY`). Draw surface is segregated into narrow per-stream `IRngStream` handles granted at the composition root (no draw-any-stream method); `Combat` draws assert `Mode == TurnBased`. Integer-only sim draws (Lemire bounded, `Math.BigMul`); **no float draw in core**. Per-entity one-shot values via pure `DeriveEntitySeed(entityId, purpose)` (recomputed, not serialized). Central self-describing snapshot (version + count + state words, little-endian) via ADR-0004's `SnapshotInto`; `Combat` stream rides every save (NOT a checkpoint-only side table). Gates: godot-csharp-specialist no-blocking; TD-ADR CONCERNS → all 5 applied (blocking seed-collision fix + segregated draw surface). See `docs/architecture/adr-0005-seeded-rng.md`. **Unblocks Save/Load quick-spec #6.**
- **Next**: All five foundation ADRs (0001–0005) are now Accepted. **Save/Load quick-spec #6 is unblocked** (ADR-0004 + ADR-0005 both exist). The only open ADR obligation is the shared ADR-0002/0004 criterion-5 target-hardware run, tracked as a pre-vertical-slice verification item. Design order resumes at GDD/quick-spec authoring (terrain-data-model GDD, #8 Colonist Entity quick-spec, Save/Load #6).
