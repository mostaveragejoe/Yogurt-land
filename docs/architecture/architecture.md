...
| ~~QQ-25~~ | ~~Checkpoint multi-owner buffer framing is unspecified~~ — **CLOSED 2026-08-26** by ADR-0004 Amendment 2026-08-26. Section-framed container: `[FileHeader][ownerId, ownerSchemaVersion, byteLength, payload]×N`, fixed write order, append-only `OwnerId` enum, section-scoped writers so no owner can overrun another, and a `buildFingerprint` checked before any payload is read. Any framing fault routes to the existing loud-fallback path — never a partial restore | technical-director | — |
| ~~QQ-26~~ | ~~Checkpoint writer lock boundary + join timeout~~ — **CLOSED 2026-08-26** by ADR-0004 Amendment 2026-08-26. The lock covers the three-state buffer swap **only**, never gzip/write/fsync, enforced by a `[ThreadStatic]` lock-depth assertion (proven against a planted violation). Both joins bounded by `CheckpointJoinTimeout` (default 5 s); on timeout the temp file is abandoned, which is safe because atomic replace leaves the previous valid slot live | technical-director | — |
...
| Checkpoint buffer framing undefined (QQ-25) | ~~HIGH~~ → **Resolved** | 7 owners shared one buffer with no container format | **CLOSED 2026-08-26** — ADR-0004 Amendment 2026-08-26 |
