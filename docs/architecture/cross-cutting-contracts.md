...
- **CD-9 (resolved: NO mid-battle saves)**: saves occur only in RealTime — a snapshot whose Mode is TurnBased is corrupt, not supported. Combat-transient state lives in encounter-scoped side tables that are structurally unserializable (never in stores). Autosave fires at switch-into-tactics and at battle end.
- RNG is **per-system seeded streams**, draw-sites constrained by contract #1; stream layout is the pending Seeded RNG ADR's deliverable and must round-trip like all other state.
