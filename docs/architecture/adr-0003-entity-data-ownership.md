...
## Status
**Accepted** (2026-07-26, user decision) — spike-validated: criteria 2–5 pass across the mode-switch, pathfinding and save/load spikes. · **Amended 2026-08-03** (Battle Persistence) · **Corrected 2026-08-24** (RealTime composite-walkability formula) · **Amended 2026-08-24** (Downed state + persisted injury, CD-13)
...
>
> The Time Authority GDD's Battle Persistence ruling overturns CD-9's no-mid-battle-save half: a rolling battle checkpoint is written after every resolved actor activation (ADR-0004, pending). **The corrected serialization rule — exact wording is binding**: *combat-transient state is never in the stores, and is serialized **only into the battle checkpoint by its owning systems** — never into a colony-mode save.* The stores/side-tables firewall is unchanged and is precisely what makes this clean: colony saves still contain zero combat state by construction; the checkpoint is a second, combat-scoped serialization consumer, not a licence to move any encounter field into `ColonistStore` (criterion 6 stands verbatim). Consequences carried by this amendment:
>
> 1. **Encounter-scoped side tables gain checkpoint-grade `Snapshot()`/`Restore()`** on their owners (Turn Order: initiative/order; Action Economy: AP; Targeting: locks) — invoked by the checkpoint writer only. "Requires no code" is retracted; the code is scoped to ADR-0004.
> 2. **`RaiderStore` serializes into the checkpoint in its entirety** — "raiders add zero records" now holds for colony-mode saves only.
> 3. **Un-reaped `IsDead` colonists and `IsBroken` doors become legal serialized states** (checkpoint only). The load path must NOT reap them — reaping remains `PostEncounterReconcile`'s job — and the `UnitOccupancyIndex` rebuild on load must filter dead units, or the TurnBased exclusivity assertion fires on a corpse cell.
> 4. **The `EncounterOutcomeInbox` needs no serialization** under ADR-0004's ordering invariant (no checkpoint is written between battle-end declaration and the reconcile drain — the inbox is provably empty at every checkpoint).
> 5. **The load window gains a mode-assertion exemption** (shared with ADR-0001's amendment): a checkpoint restore in `Mode == TurnBased` writes RealTime-only field groups.
> 6. The Allocation Policy's "one-shot allocation at the CD-9 autosave moment is accepted outright" is **narrowed to colony-mode autosaves**; checkpoint entity snapshots ride ADR-0004's double-buffered async path (Option A).
>
> The write-ownership table, occupancy index, reservation gating, doors contract, and `EncounterOutcomeReport` machinery are unchanged and remain authoritative.
...
- CD-9 (save half **overturned by Battle Persistence, 2026-08-02** — see Amendment 2026-08-03; battle-length half stands): combat-transient state is never in the stores and is serialized only into the battle checkpoint by its owning systems — never into a colony-mode save
...
| Health / body | `Hp`, `IsDead`, **`IsDowned`, `BleedOutRemaining`, `Injury`, `Stage`, `MobilityFactor`, `RecoveryRemaining`, `HasProsthetic`** *(amended 2026-08-24 — see the Amendment below)* | Needs & Simulation (decay, rest, recovery, **bleed-out tick, injury recovery, and injury application at reconcile**) | Combat: Targeting & Resolution (damage) | **The decided arbitration**: writer-per-authority, mirroring ADR-0002 rule 4. One legal writer at any moment; authority-tagged interfaces + mode assertion enforce it. Neither system ever calls the other. **`Hp`→0 sets `IsDowned`, not `IsDead`** (amended); the store internally removes dead units from the occupancy index (dead units do not block cells — **downed units still do**) |
...
> ### Amendment 2026-08-24 — Downed state and persisted injury (CD-13; authored with `design/quick-specs/colonist-entity-attributes.md`)
>
> CD-13 requires that felled colonists be stabilized rather than revived, and names **Colonist Entity #9** as owner of the persisted-injury half via this ADR's reconcile seam. That needs a state between alive and dead, which the health row above did not have. Changes:
>
> 1. **`Hp`→0 sets `IsDowned`, not `IsDead`.** Death is a later, separate transition. The original row's "`Hp`→0 sets `IsDead` via the same lethal write" is **retracted**.
> 2. **`BleedOutRemaining` is denominated in sim-ticks and decrements under BOTH authorities** — by `TicksPerTurn` at the downed unit's turn start in TurnBased, per elapsed tick in RealTime. Reaching 0 is death, in whichever mode it runs out.
> 3. **Downed state is store state, not a combat side table.** The clock deliberately outlives the encounter: a colonist downed late in a battle walks out still downed and still rescuable, and the colony has the remaining window to reach them. Because it stays meaningful outside an encounter, the firewall rule points it at the store, and it serializes into colony saves like any other health field. **This is not a weakening of the firewall** — genuinely encounter-scoped state (initiative, AP, target locks) is unaffected.
> 4. **`PostEncounterReconcile` does not resolve downed colonists.** It applies injuries for those stabilized during the battle, increments `BattlesSurvived`, and reaps the dead. Colonists still downed are left downed with their clock intact.
> 5. **Occupancy**: downed units still occupy their cell and remain legal targets. Only *dead* units are removed from `UnitOccupancyIndex`.
> 6. **No new writer.** Needs & Simulation, already the RealTime health writer, also ticks bleed-out and recovery and applies injury outcomes at reconcile. Combat: Targeting & Resolution remains the TurnBased writer.
>
> Field semantics, severity tiers, recovery paths and tuning live in the quick-spec; this ADR fixes only ownership and the state machine.

**Combat-transient state is NOT colonist state.** Initiative, turn order position, action points, selected targets, overwatch flags — everything that exists only during an encounter — lives in **encounter-scoped side tables owned by the combat systems that rule them** (Turn Order owns initiative/order; Action Economy owns AP; Targeting owns target locks), keyed by `EntityId`, created at switch-in from store reads, discarded at battle end. They never enter a colony-mode save — the colonist store snapshot simply never contained battle state — and *[amended 2026-08-03 — Battle Persistence]* they are serialized **only into the battle checkpoint by their owning systems** (checkpoint-grade `Snapshot()`/`Restore()`, invoked by the checkpoint writer alone; ADR-0004). The firewall mirrors ADR-0002's and is unchanged: **any proposed `ColonistStore` field that is meaningless outside an encounter belongs in a combat side table.**
...
>
> So the formula was the defect, not the behaviour — the implemented and spike-verified rule was always the bracketed one. **Nothing downstream needs to change**: the pathfinding spike tested the correct rule, and the Pathfinding quick-spec (§4) was authored against it. This correction only makes the ADR say what the ADR already meant.
>
> **Why it matters that it read the other way.** Doors blocking in RealTime is not a cosmetic slip: it is the traffic-deadlock case. A colony whose colonists refuse to path through their own closed doors stalls on the first door it builds, and the pathfinding spike names auto-open as precisely what prevents that. An implementer following the formula literally would have shipped the deadlock and then debugged it as a pathfinding bug.
>
> **Scope**: this is a wording correction to an Accepted ADR, not a design change — no re-validation, no propagation, no status change. TurnBased is untouched and still blocks on closed doors (opening is an action, Combat set). The one live caveat is recorded as Pathfinding quick-spec §8 item 3: **if a "door policy" that forbids opening ever ships** (CD-16 lists door policy among peacetime standing decisions), RealTime gains a blocking case again and this rule needs revisiting.
...
- `Snapshot()` allocation stance (symmetry with ADR-0002's spike-deferred buffer strategy): entity snapshots are kilobytes — a one-shot allocation at the colony-mode autosave moments is accepted outright; no buffer-reuse machinery is warranted at this scale. *[Narrowed 2026-08-03 — Battle Persistence]*: the per-activation battle checkpoint instead rides ADR-0004's double-buffered async path (Option A).
...
- The stores/side-tables firewall keeps colony saves structurally free of combat state: combat-transient state never enters the stores, so a colony-mode save contains no battle state by construction. *[Amended 2026-08-03 — Battle Persistence]*: the original "requires no code — there is nothing to skip" claim is retracted; the battle checkpoint requires checkpoint-grade serialization on the side-table owners, scoped in ADR-0004.
...
| CD-9 (systems index) | Save half overturned by Battle Persistence (2026-08-02); battle-length half stands | Colony saves carry zero combat state; the battle checkpoint carries it via its own writer | Combat-transient side tables outside the stores — never in a colony-mode save; checkpoint-serialized by their owners only (Amendment 2026-08-03, ADR-0004); the outcome inbox stays unserialized under ADR-0004's ordering invariant |
...
Also validated here: ADR-0002's stair Z-linkage; A* is allocation-free (0.00 B/query) and deterministic. **Movement model decided by the user 2026-07-26**: 8-connected, corner-cutting banned, integer octile costs (10/14) and octile heuristic — a game rule, recorded for the Pathfinding quick-spec, with no ADR contract affected. Verified that a one-cell-thick diagonal wall still seals (chokepoint play survives diagonals) and that the region index's connectivity matches the pathfinder's exactly. **One constraint found, routed to the Pathfinding quick-spec, not an ADR change**: a full region flood fill costs 4.16 ms (25.1% of a frame) and must never run per dig — it needs an incremental, deferred, or per-layer rebuild trigger.
...
## Related Decisions
- ADR-0001 Time Authority (**Accepted** 2026-07-26) — mutation window, mode exclusivity that makes writer-per-authority sound, `ParticipantIds`, `PostEncounterReconcile` as the outcome-processing site; companion edits listed in Migration Plan
- ADR-0002 Terrain Data Model (**Accepted** 2026-08-24) — firewall table pre-assignments honored (occupancy, reservations, doors; zones stay Stockpile & Hauling's sparse cell-sets); `IsPassableTerrain` composed with door/occupancy inputs under the mode-aware rule; enforcement patterns reused (writer tables, debug sweeps, single write paths, `Revision`)
