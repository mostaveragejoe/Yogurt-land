...
- **To creative-director, before the Needs & Simulation GDD**: when a battle ends, does the colony resume as if zero time passed, or does colony time advance by the battle's represented duration (as N catch-up sub-steps)? Both remain architecturally possible under fixed-dt sub-stepping; delta-scaling would have closed the second option, which is (part of) why it is banned. **RESOLVED 2026-08-07 (user ruling, systems-index CD-19)**: colony time advances by a **turn-derived, capped** amount as N ordinary catch-up sub-steps, ordered AFTER `PostEncounterReconcile` and BEFORE the battle-end autosave (a save never holds pending time debt). Rate/cap and the guardrails (multiplicative decay only, clemency clause, single re-plan) are owned by Needs & Simulation #13. This is the option delta-scaling's ban was preserved for — the architecture holds as designed.
