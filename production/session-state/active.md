...
<!-- STATUS -->
Epic: Pre-Production
Feature: Gate check closed 2026-08-24 — Gate 1 CONCERNS (ratified), Gate 2 FAIL (expected)
Task: SESSION CLOSED. Next session, pick one: (a) /create-architecture — last missing Technical Setup artifact and a hard Production blocker, needs its own session; (b) define the vertical slice (1 stratum, ~3 colonists, dig+build, one raid, real rebuild cost, prototypes/, ADR-exempt, answers CD-18 only); (c) amend ADR-0005 for the re-roll ruling. CHECK CI FIRST — main was pushed at 46e65d8 and the workflow has never run before.
<!-- /STATUS -->
