...
Task: ADR-0005 Seeded RNG COMPLETE 2026-08-07 (Proposed; gates run; B3 = defer-to-#18 with identical-replay default, user confirmed; companion edits ADR-0001/0004 + technical-preferences + systems-index AND registry update all applied with user approval). Next: Colonist Entity quick-spec (#9); Save/Load #6 spec-level unblocked; /architecture-review in a FRESH session to validate ADR coverage
<!-- /STATUS -->
...
- **B3 (reload seed policy, CD-GDD-ALIGN M1 obligation)**: discharged at ADR level — identical replay is the placeholder default by construction, `ReseedOnLoad` knob (off) named, **design pick routed to Raid Trigger #18's GDD** (user confirmed the deferral on re-prompt). **#18 authoring must surface this choice.**
- **User approvals on re-prompt (all three)**: keep B3 deferral; apply all four companion edits; apply the registry update. **APPLIED 2026-08-07**: ADR-0001 (status stamp, Enables, `long EncounterId` + `EncounterIdSource` comment, RNG rule + load window, Related Decisions), ADR-0004 (Depends On, content item 3 discharged + owning-combat-systems wording, risk note, Related Decisions), technical-preferences (existing RNG forbidden pattern extended + 3 new patterns + ADR-0005 log entry + Next line), systems-index (#4 → ADR authored), registry (revised `rng_outside_authority_execution` w/ old value in comment; new: `rng_streams`, `rng_stream_grants`, `simulation_rng`, `rng_rederivation_at_restore`).
- **Reminder**: run `/architecture-review` in a FRESH session (never the authoring session) to validate ADR coverage.
