...
| 2 | ~~**Dig time's home.**~~ — **CLOSED 2026-08-24.** `DigCost` lives in the catalog (user decision) so every tier-ordered number sits in one load-validated table; Excavation keeps the accumulation *rule*. The terrain GDD's Tuning Knobs table has been corrected to split the **number** (catalog) from the **rule** (Excavation), and its tier-ordering row's "nobody cross-checking direction" warning is marked **discharged** — C3 is that cross-check | Terrain Data Model (#1) — **done** | — |
