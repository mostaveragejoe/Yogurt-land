...
**A battle is a single sitting.** CD-9's no-mid-battle-save rule and the 8–15 minute battle budget (20-minute hard ceiling) are stated here as design, not constraint: you cannot pause the judgment, save inside it, or pick it apart across sessions — the battle asks to be answered now, in one breath. *(What this deliberately does not claim, per CD-GDD-ALIGN M1: a determined player can quit and relaunch to face the raid again from its start — EC-8. That is accepted under the anti-permadeath anti-pillar and the genre's norms, not denied in prose the spec can't back; EC-8 records the handling and the named upgrade paths.)*
...
- **If the player quits to desktop mid-battle**: on relaunch they load the switch-in autosave — the colony resumes at the moment combat began, and a raid begins again from that state. Forced by CD-9 (no mid-battle save exists to return to). A **confirmation dialog** states the consequence without advertising a rewind: *"Quitting now abandons the battle — the colony returns to the moment the raid began."* **Design debt, recorded (CD-GDD-ALIGN M1)**: this is a designed-in retry, accepted in MVP under the anti-permadeath anti-pillar and genre norms (RimWorld/DF/Battle Brothers all permit the equivalent reload). Whether the reloaded encounter **replays identically or re-rolls within the same threat band** is deliberately NOT decided here — with deterministic seeds an identical replay hands the player perfect foreknowledge, crossing CD-15's information ceiling; the decision belongs to **Raid Trigger (#18) + the pending Seeded RNG ADR** (production seed policy, distinct from AC-30's fixed-seed test reproducibility). Named upgrade path if playtests show retry abuse: **suspend-to-exit as a single non-reloadable state** — CD-9's own pre-authorized post-MVP option. *(EC-8)*
...
- **If Squad Prep drafts zero colonists when a raid fires** (all dead, incapacitated, or unreachable): **the switch proceeds anyway** — an empty-roster encounter is legal, and Combat resolves the unopposed raid inside turn-based mode (user decision 2026-08-02, chosen over a Raid-Trigger pre-check). **Obligation transmitted to the Combat set (#19–23)**: its turn loop must handle zero player-controlled units — raiders act, the player observes; the identical exit path still applies. **CD constraint (CD-GDD-ALIGN C6)**: an unopposed encounter is a consequence being applied, not a test — the Combat set may **fast-resolve or skip** a zero-player-unit encounter without violating Rule 7's identical exit path; the player must not be made to watch up to 20 minutes of zero-agency raiding. CD-3's survivability floor remains #18's to define — this edge case is not a licence to skip it. *(EC-10)*
...
- **Quit-mid-battle confirmation** (#27 or shared shell UI): *"Quitting now abandons the battle — the colony returns to the moment the raid began"* — blocking, consequence-stating dialog, combat mode only (EC-8, AC-43).
- **Manual save control** (shared shell): visibly disabled — inert, not hidden — for the entire duration of combat; re-enabled on return (Rule 9, CD-GDD-ALIGN C5, AC-44).
...
14. **[Rule 9]** GIVEN either autosave moment, WHEN the save call fires, THEN no save-slot UI prompt/event is raised (silent) — verified against a stub UI sink; full serialization fidelity is AC-54.
...
43. **[EC-8]** GIVEN a battle is in progress, WHEN the player initiates quit-to-desktop, THEN a confirmation dialog displays consequence-stating text substantially matching "Quitting now abandons the battle — the colony returns to the moment the raid began" and quitting is blocked until confirmed. Verify the negative case too: no such dialog appears when quitting during colony play.
44. **[Rule 9, CD-9 — CD-GDD-ALIGN C5]** GIVEN combat is active, WHEN the player attempts a manual save via its normal control, THEN the save control is visibly disabled (inert, not hidden) and no save occurs; on return to colony play the control is re-enabled.
45. **[EC-8]** GIVEN the player confirms quit mid-battle and relaunches, WHEN the game loads, THEN the colony resumes at the moment combat began and the encounter restarts from its beginning. (Straddles manual/integration — save *content* fidelity is AC-54; this criterion checks only the observable in-game behavior.)
...
49. **[Rule 2 — BLOCKED: Raid Trigger #18]** GIVEN Raid Trigger's threshold is reached, WHEN it calls `RequestSwitch`, THEN the switch is accepted and the encounter begins with correct breach cells/participants end-to-end.
...
52. **[EC-10 — BLOCKED: Squad Prep #24 + Combat set #19–23]** GIVEN Squad Prep drafts zero colonists for a triggered raid, WHEN the switch proceeds anyway, THEN Combat's turn loop correctly handles zero player-controlled units (raiders act, player observes, exit path unchanged).
53. **[ADR Correction 2 — BLOCKED: Squad Prep #24]** GIVEN multiple colonists co-located on one cell at the pending-switch moment, WHEN pre-switch placement normalization runs, THEN each unit decides against cells already claimed by earlier decisions in the same pass (not live occupancy), and only the correct subset moves. This is the specific implementation trap named in ADR-0001 Correction 2 — must be an explicit regression test once #24 exists.
54. **[Rule 9/EC-6/EC-8 — BLOCKED: Save/Load #6]** GIVEN the two autosave moments, WHEN Save/Load's real serialization runs, THEN the resulting files load correctly and reproduce the exact pre-swap / post-reconcile state.
55. **[Rule 10 — BLOCKED: Notifications]** GIVEN notifications queued during combat, WHEN the colony view returns, THEN all display with correct content/priority per Notifications' own rules.
...
|---|---|---|---|
| 1 | Post-battle time semantics: zero-elapsed vs. advance-by-battle-duration (as N catch-up sub-steps). Both stay architecturally open; AC-59 is unwritable until decided. | creative-director | **Before the Needs & Simulation GDD (#13)** — already routed via ADR-0001 |
...
| 3a | Reload seed policy for the switch-in autosave (EC-8, M1): identical replay vs. re-roll within the threat band — an identical replay crosses CD-15's information ceiling on retry. | Raid Trigger (#18) + Seeded RNG ADR | Seeded RNG ADR authoring (next ADR in the log) |
