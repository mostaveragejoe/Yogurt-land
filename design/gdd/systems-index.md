...
| 5 | Material Catalog (inferred) | Economy | MVP | **Drafted** 2026-08-20 — tier-ordering invariant + CD-6 EV rule both load-validated; 3 materials registered | Quick-spec | [../quick-specs/material-catalog.md](../quick-specs/material-catalog.md) | (none — Foundation) |
...
| 8 | Pathfinding & Navigation (inferred) | Core | MVP | **Drafted** 2026-08-20 — spike-backed (44/44); region-rebuild trigger, path ownership and door cost all decided | Quick-spec (spike + ADR carry it) | [../quick-specs/pathfinding-navigation.md](../quick-specs/pathfinding-navigation.md) | Terrain Data Model, World Change Event Bus |
...
| 9 | Material Catalog | Quick-spec — ✅ **Drafted** 2026-08-20 | MVP | Foundation | economy-designer | S |
| 10 | Pathfinding & Navigation | Quick-spec — ✅ **Drafted** 2026-08-20 | MVP | Core | godot-specialist | S |
...
| Design docs written | **4** — Terrain Data Model (Full GDD, **Approved** 2026-08-02); Time Authority / Mode-Switch (Full GDD, **Designed**; third `/design-review` pass now unblocked, see row #2); Pathfinding & Navigation (Quick-spec, **Drafted** 2026-08-20); Material Catalog (Quick-spec, **Drafted** 2026-08-20) |
...
| MVP systems designed | **4/25** documented (2 full GDDs + 2 quick-specs). Separately, 2 more MVP systems are **ADR-specified** rather than doc-designed: #4 Seeded RNG (ADR-0005) and #6 Save/Load battle-checkpoint half (ADR-0004) — per the routing policy, ADR-only is a complete tier, not a gap |
...
- [ ] **Quick-spec batch (design order #8–#11)** — ✅ Pathfinding & Navigation and ✅ Material Catalog both **Drafted 2026-08-20** (tier template at `.claude/docs/templates/quick-spec.md`). Remaining: **Colonist Entity & Attributes** (#8 — highest fan-out of the four), **Terrain Rendering & Cutaway** (#11 — folds in the ADR-0002 damage-overlay draw-call open item)
- [ ] **Clarify dig-time ownership in the terrain GDD's Tuning Knobs table** — Material Catalog now owns `DigCost` (user decision 2026-08-20) so every tier-ordered number sits in one load-validated table; Excavation keeps the accumulation *rule*. Substance agrees, wording does not. Corrected already in `design/registry/entities.yaml`; still owed in `design/gdd/terrain-data-model.md`. Trigger: its next `/design-review`, or before #15/#16 is authored — whichever is first
...
- [ ] **Amend ADR-0003** — its RealTime composite-walkability formula makes a closed door block, contradicting its own bracketed note and the pathfinding spike's verified behavior. Recorded in the Pathfinding quick-spec (§4 flag, §8 item 4); not yet fixed in the ADR
- [ ] **Target-hardware criterion-5 run** — the shared promotion gate for ADR-0002 + ADR-0004 (+ ADR-0005 alongside). Re-run `prototypes/terrain-spike/render/` on real hardware; software Vulkan (lavapipe) gave 3–4 fps and no usable signal. Then re-run `/architecture-review` in a **fresh** session
