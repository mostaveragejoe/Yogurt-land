...
- [x] After spikes report: begin GDD authoring per the design order (`/design-system terrain-data-model`) — **CONFIRMED COMPLETE 2026-08-07**: terrain-data-model.md was already Approved (independent /design-review, two rounds, all findings resolved) before this checklist item was marked; the branch consolidation on 2026-08-06 carried the completed doc into `main` without updating this line. No authoring action was taken this session.
- [ ] **Next gap**: Time Authority / Mode-Switch GDD is authored and status **Designed**, but has never had an independent `/design-review` pass (unlike Terrain Data Model). Run `/design-review design/gdd/time-authority-mode-switch.md` in a **fresh session** (never the same session as authoring, per the skill's independence rule).
