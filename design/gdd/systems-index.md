...
| 4 | Seeded RNG / Determinism (inferred, TD) | Core | MVP | **Designed** (ADR-0005 Proposed 2026-08-07; godot-specialist + TD-ADR gates: CONCERNS → all findings applied same day) | ADR-only | [ADR-0005](../../docs/architecture/adr-0005-seeded-rng-determinism.md) | (none — Foundation) |
...
## Creative Director Notes (routed questions ruled 2026-08-07 — recommendations by creative-director review, RULINGS by user)

- **CD-19 (post-battle time: turn-derived advance with cap — USER RULING 2026-08-07, overrides the CD recommendation of zero-elapsed + labor aftermath)**: when a battle ends, colony time advances by an amount derived from the battle's turn count, **capped**, executed as N ordinary catch-up sub-steps. Rate and cap are **Needs & Simulation #13** tuning knobs (never hardcoded). Binding guardrails carried from the CD analysis: decay uses ONLY the multiplicative form (`RatePerDay × elapsed ÷ 1440`) so catch-up is arithmetically identical to normal accrual — one clock, no second code path; **ordering pin** — catch-up runs AFTER `PostEncounterReconcile`, BEFORE the battle-end autosave (a save never holds pending time debt; Rule 8/AC-10 revision required in the time-authority GDD); **clemency clause** — a single catch-up injection may never push a need past a lethal threshold; dead colonists are reaped before catch-up (no ghost decay); threshold crossings during catch-up queue per Rule 10 and present after the survey; **Job Assignment #10** re-plans ONCE after catch-up, not per sub-step. **Recorded risks to test in MVP playtests, not dismissed**: double-punish on a lost raid, the 3×-speed triple-tax, and the rush incentive inside a mode whose anti-pillar forbids time pressure (routed to the Combat set's pacing-control obligation, C3). Closes ADR-0001's first open question; unblocks **#13**.
- **CD-20 (colony-save save-scum: raid-window lock + reload re-roll, Commitment Mode opt-in — USER RULING 2026-08-07, adopting the CD recommendation)**: manual saves are **inert (not hidden), with a labelled reason**, from raid warning through battle-end — the newest player-selectable save entering a fight is always one the game wrote; save-and-quit during the window writes a **non-selectable** slot that Continue resumes. Loading a pre-raid save **re-rolls breach point and raider composition within CD-15's ceiling** — a rewind retakes a *different* test, never replays a known answer key (obligation: raid composition/breach are not baked into pre-warning colony saves — owners **Raid Trigger #18** + ADR-0005 streams). **Commitment Mode** (single rolling save, no manual saves) is a per-colony new-game toggle, **default OFF**, immutable after creation. Design intent of record: rewinding is not impossible (tampering and the ADR-0004 corruption fallback reach it) — it must **cost more than losing**. Falsification: ≥2 of 8 MVP playtesters reloading the same raid more than once = friction too low (escalate); frustration at the lock window = too wide (narrow toward warning+N-minutes). The time-authority GDD's "no pre-battle rewind by any player action" absolutism is REPLACED by the adopted honest-restatement paragraph (full text in `production/session-state/active.md`; lands via `/propagate-design-change`). **Save/Load #6 obligations**: slot taxonomy (manual + colony autosaves + battle-end autosave selectable; rolling checkpoint Continue-only; switch-in autosave fallback-only), lock window with labelled reason (AC-44 extends to the warning window), load-list "before raid #N" labelling, per-colony Commitment flag. Owners: **Save/Load #6**, **Raid Trigger #18**, time-authority GDD, **Notifications** (lock-reason display).
