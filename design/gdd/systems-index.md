...
3. **World Change Event Bus** — one publisher (Terrain), many subscribers. Capped by ADR: a dumb synchronous dispatcher — no filters, no replay, no queueing. **Ordering corrected 2026-08-25 (ADR-0006)**: this line previously read *"no priorities … or ordering guarantees"*, which was stricter than cross-cutting contract #3 (*"no priorities beyond declared handler order"*) and stricter than ADR-0002 rule 8, which **requires** a stable order for deterministic event streams. Subscribers register with a declared integer priority; ordering is deterministic but **not load-bearing** (ADR-0006 rule 12 forbids cross-subscriber reads during dispatch).
...
| ADRs written | **5** + contracts annex — **ADR-0001 & ADR-0003 Accepted** (2026-07-26); **ADR-0002 Accepted 2026-08-24**; ADR-0004 and ADR-0005 Proposed. ADR-0002/0004 share one target-hardware criterion-5 promotion gate; ADR-0005 has no hardware gate of its own but promotes with ADR-0004 |
...
- [ ] **Vertical slice — the next real commitment.** Deliberately scoped **below** the MVP bar so it is buildable against the specs that exist: one stratum, ~3 colonists, two job types (dig and build), one raid with one raider type, one breach. **Rebuild cost is the one thing not simplified** — real materials, real colonist-hours. Lives under `prototypes/`, **exempt from ADR ceremony** like the five Tier 0 spikes. It answers exactly one question, CD-18's: *with real cost attached, does the rebuild loop close fast enough that scars teach rather than punish?* The fun spike could not answer this — it had free, instant rebuilding
- [x] **Amend ADR-0005 for the re-roll ruling** — **DONE 2026-08-26** (ADR-0005 Amendment 2026-08-26). Combat stream derives from `splitmix64(RootSeed, Combat, EncounterId, EncounterAttempt)`; the counter lives in a `user://` profile file, never the colony save. No schedule cost: resume determinism restores `State` directly and never re-derives, so Battle Persistence is unaffected, and TR-time-026 keeps full strength with `EncounterAttempt` as a declared determinism input. New obligation on Debug Console (#29): read/pin `EncounterAttempt` for QA reproducibility
- [ ] **New systems-index entries needed** — Research/Technology and a production chain, both required by the prosthetics-as-unlockable-technology ruling (2026-08-24). Also furniture/workstations, which the supply chain will need
- [ ] **`/create-architecture`** — the master architecture document is the last missing Technical Setup artifact and a hard blocker at the Production gate. Needs its own session; most of it re-indexes material that already exists, but two outputs are genuinely new: a consolidated open-questions register and an API boundary table
- [ ] **Checkpoint clause — now ADR-0004's gate, no longer ADR-0002's.** Criterion 5's remaining half: checkpoint snapshot+write at per-activation combat cadence on ADR-0004's double-buffered async path. **Needs that path built first** — it does not exist. Once measured, promote the three ADRs together and re-run `/architecture-review` in a **fresh** session
- [x] **`/test-setup` — DONE 2026-08-24.** xUnit suite at `tests/Hollowdeep.Tests.csproj` (plain .NET, no engine — the core is Godot-free by contract), `Hollowdeep.sln`, one real example test, smoke-gate seed, and `.github/workflows/tests.yml` with **two ADR-mandated grep gates**: core-is-Godot-free (ADR-0001/0002/0003) and no-stock-RNG (ADR-0005). Framework and coverage slots in technical-preferences filled. **3 of the 5 `/gate-check` pre-gate artifacts now exist**
- [x] **`/ux-design` — DONE 2026-08-24.** Both remaining pre-gate artifacts written: `design/accessibility-requirements.md` (**WCAG-AA adapted for games** committed as the tier; 11 sections) and `design/ux/interaction-patterns.md` (11 patterns P1–P11, authored ahead of any screen spec because the GDDs had already pinned them). **All 5 `/gate-check` pre-gate artifacts now exist — the phase gate is reachable for the first time**
- [ ] **Run `/gate-check`** — now unblocked. Expect CONCERNS rather than PASS: no screen UX specs exist yet, and `/ux-review` has never run against the new tier
- [ ] **Blueprint UI (#26) carries the highest accessibility load in the project** (per-feature matrix) — worth knowing before its UX spec is written. It owns the click-click equivalent, the interrupted-drag rule, the disabled-state treatment and the on-attempt gesture affordance
