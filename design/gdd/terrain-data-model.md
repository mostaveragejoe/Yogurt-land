...
> **Status**: Designed — `/design-review` 2026-08-02 verdict **NEEDS REVISION**, all 7 blocking items + 13 recommended revisions applied same day; pending re-review
...
**Defensive contract** *(added per /design-review 2026-08-02, symmetric with Formula A)*: `R < 0` is **rejected, not clamped** — debug assertion; production performs no state change, publishes nothing, returns `RejectedInput` with Δ = 0. Rejection happens before any arithmetic or cast, so the previously possible failure — a negative R wrapping through the `ushort` store into massive overheal — is unreachable. The stated output range below is a *guarantee*, not an assumption about callers.
...
|---|---|---|---|
| 1 | **60 fps on target hardware.** The terrain spike ran on software Vulkan (3–4 fps — no signal). Draw calls, memory and CPU costs are hardware-independent and pass; the frame-rate clause does not yet exist. **Status split (2026-08-02)**: non-blocking for the plain-C# core; blocking for ADR-0002's promotion and for the render backend (see Known Gaps #1). | technical-director | **This is the last gate before ADR-0002 moves from Proposed to Accepted.** Run `prototypes/terrain-spike/render/` on target hardware — now alongside the 4.7.1 GridMap verification gate (Visual/Audio Requirements). |
