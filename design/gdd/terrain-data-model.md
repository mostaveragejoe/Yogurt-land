...
1. **No hidden gate.** Every wall is diggable by any colonist (C2). The plan the player draws is never silently refused for a reason they cannot see. Tier changes how long work takes, never whether it is allowed.
...
| Tier ordering invariant (dirt < granite < reinforced) | **Material Catalog (#5) owns it as a pillar-level constraint**; acceptance criteria in #5 (HP) and #15/#16 (dig time) | Terrain never compares tiers — it has no tier field. **But the invariant is a Pillar 3 requirement**: "granite held where dirt failed" only teaches if the ordering is reliable. It is currently split across two owners with nobody cross-checking direction, which is how invariants die — #5 owns the cross-check, and `/consistency-check` should verify it |
...
- **If a wall completes in a cell a colonist occupies**: the wall is placed and the colonist is displaced per the rule above. *Blocking the job instead would let a single idling colonist stall construction indefinitely; the brief visual of being moved out of rock is the cheaper cost.* **Presentation note for #7**: per Pillar 4's Design Test B (visible over abstracted), displacement should read as a shove or step-out, not a teleport — a colonist popping out of rock is an ant-farm legibility loss.
...
| 6a | **Map-authoring constraint (not deferrable)**: with stairs down-only, an authored region reachable *only from below* would be permanently unconnectable. **No authored region may be reachable only from below.** | Map Authoring / Content Load (#14) | **Before the first map is authored** — this is a hard constraint, not a mechanic question. |
