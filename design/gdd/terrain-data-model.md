...
|---|---|---|---|
| 1 | **60 fps on target hardware.** The terrain spike ran on software Vulkan (3–4 fps — no signal). Draw calls, memory and CPU costs are hardware-independent and pass. **Re-scoped 2026-08-07 (user decision)**: this is no longer a promotion gate. ADR-0002 is Accepted; the target-hardware run is a pre-vertical-slice verification item. | technical-director | **Run before the vertical slice.** Run `prototypes/terrain-spike/render/` on target hardware — alongside the 4.7.1 GridMap verification gate (Visual/Audio Requirements). |
